(function (w, $) {
     ...
     *
     * @constructor
     * @param {Object} element, this id will be used to create jQuery selector
     ...
     * }
     */
    w.Mrs = function (element, userOptions) {
        //var _self = this;
        elementId = 'mrs' + Math.ceil(Math.random()*100000000000000);
        var _slider;
        var _options = {
            min: 0,
            max: 1440,
        ...
        function _addScale() {
            ...
            $(element).parent().prepend('<div id="steps_' + elementId + '" class="MrsSteps"></div>');
            ...
            //
            $(element).width(nSteps * stepWidth + '%');
        }
        ...
        function _refreshHandles() {
            ...
            for (var index in values) {
                handles.eq(index).html('<i class="fa fa-eject"></i><br /><span class="MrsHandleLabel">' + _options.handleLabelDispFormat(values[index]) + '</span>');
            }

            _markRangeOnScale(handles.parent());
            _toggleHandles(values.length);
        }

        function _markRangeOnScale(_ele) {

            var steps = $(_ele).parent().find('.MrsStep')
            //reset color
            steps.css('background-color', 'rgba(228, 234, 234, 0.5)');
            ...
            steps.each(function () {
                ...
                for (var i = 0; i < periods.length; i++) {
                    if (dataStart >= periods[i].start && dataStart < periods[i].stop) {
                        $(this).css('background-color', 'rgba(26, 188, 156, 1)');
                    }
                }
            });
        }
